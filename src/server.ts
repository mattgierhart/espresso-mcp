...
export function createServer(): McpServer {
  const server = new McpServer(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions:
        "Use these tools to find great espresso cafes worldwide. The curated database focuses on specialty third-wave coffee shops — places that source carefully, roast with intent, and pull espresso seriously. Cafes that hide bad coffee behind flavored syrups score poorly. Pass coordinates to find_espresso_near, or call score_cafe with observed signals to evaluate an arbitrary shop.",
    },
  );
